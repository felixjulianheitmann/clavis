...
class PendingDownloadsList extends StatelessWidget {
  const PendingDownloadsList({super.key});

  @override
  Widget build(BuildContext context) {
    final translate = AppLocalizations.of(context)!;
    return ExpansionTile(
      title: Text(translate.page_downloads_pending_title),
      children: [_List()],
    );
  }
}
    ...
    return BlocBuilder<DownloadBloc, DownloadState>(
      builder: (context, state) {
        final items = state.dlContext.pendingOps.map((op) {
          return _PendingListItem(operation: op);
        });
...
class _PendingListItem extends StatelessWidget {
  const _PendingListItem({required this.operation});
